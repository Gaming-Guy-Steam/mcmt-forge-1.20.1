...
public class ProxiedChemicalTankHolder<CHEMICAL extends Chemical<CHEMICAL>, STACK extends ChemicalStack<CHEMICAL>, TANK extends IChemicalTank<CHEMICAL, STACK>>
   extends ProxiedHolder
   implements IChemicalTankHolder<CHEMICAL, STACK, TANK> {
   private final Function<Direction, List<TANK>> tankFunction;

   public static <CHEMICAL extends Chemical<CHEMICAL>, STACK extends ChemicalStack<CHEMICAL>, TANK extends IChemicalTank<CHEMICAL, STACK>> ProxiedChemicalTankHolder<CHEMICAL, STACK, TANK> create(
      Predicate<Direction> insertPredicate, Predicate<Direction> extractPredicate, Function<Direction, List<TANK>> tankFunction
   ) {
   ...
   public List<TANK> getTanks(@Nullable Direction side) {
      return this.tankFunction.apply(side);
   }
}
