...
public class SlurryInventorySlot extends ChemicalInventorySlot<Slurry, SlurryStack> {
   ...
   public static SlurryInventorySlot drain(ISlurryTank slurryTank, @Nullable IContentsListener listener, int x, int y) {
      ...
      Predicate<ItemStack> insertPredicate = getDrainInsertPredicate(slurryTank, SlurryInventorySlot::getCapability);
      return new SlurryInventorySlot(
         slurryTank, insertPredicate.negate(), insertPredicate, stack -> stack.getCapability(Capabilities.SLURRY_HANDLER).isPresent(), listener, x, y
      );
   }

   private SlurryInventorySlot(
