...
public class TileEntityPressurizedTube extends TileEntityTransmitter implements IComputerTile, ITileRadioactive {
   private static final Collection<Capability<?>> CAPABILITIES = Set.of(
      Capabilities.GAS_HANDLER, Capabilities.INFUSION_HANDLER, Capabilities.PIGMENT_HANDLER, Capabilities.SLURRY_HANDLER
   );
   ...
   public TileEntityPressurizedTube(IBlockProvider blockProvider, BlockPos pos, BlockState state) {
      ...
      this.addCapabilityResolver(
         this.gasHandlerManager = new ChemicalHandlerManager.GasHandlerManager(
            this.getHolder(BoxedPressurizedTube::getGasTanks), new DynamicChemicalHandler.DynamicGasHandler(this::getGasTanks, canExtract, canInsert, null)
         )
      );
      this.addCapabilityResolver(
         this.infusionHandlerManager = new ChemicalHandlerManager.InfusionHandlerManager(
            this.getHolder(BoxedPressurizedTube::getInfusionTanks),
            new DynamicChemicalHandler.DynamicInfusionHandler(this::getInfusionTanks, canExtract, canInsert, null)
         )
      );
      this.addCapabilityResolver(
         this.pigmentHandlerManager = new ChemicalHandlerManager.PigmentHandlerManager(
            this.getHolder(BoxedPressurizedTube::getPigmentTanks),
            new DynamicChemicalHandler.DynamicPigmentHandler(this::getPigmentTanks, canExtract, canInsert, null)
         )
      );
      this.addCapabilityResolver(
         this.slurryHandlerManager = new ChemicalHandlerManager.SlurryHandlerManager(
            this.getHolder(BoxedPressurizedTube::getSlurryTanks),
   ...
   public CompoundTag m_5995_() {
      ...
      if (this.getTransmitter().hasTransmitterNetwork()) {
         BoxedChemicalNetwork network = this.getTransmitter().getTransmitterNetwork();
         updateTag.m_128365_("boxedChemical", network.lastChemical.write(new CompoundTag()));
   ...
   private <CHEMICAL extends Chemical<CHEMICAL>, STACK extends ChemicalStack<CHEMICAL>, TANK extends IChemicalTank<CHEMICAL, STACK>> IChemicalTankHolder<CHEMICAL, STACK, TANK> getHolder(
      BiFunction<BoxedPressurizedTube, Direction, List<TANK>> tankFunction
   ) {
      return direction -> {
         BoxedPressurizedTube tube = this.getTransmitter();
         return (direction == null || tube.getConnectionTypeRaw(direction) != ConnectionType.NONE) && !tube.isRedstoneActivated()
            ? tankFunction.apply(tube, direction)
