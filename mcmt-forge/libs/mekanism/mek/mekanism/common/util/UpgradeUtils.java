...
public class UpgradeUtils {
   ...
   public static List<Component> getInfo(BlockEntity tile, Upgrade upgrade) {
      ...
      if (tile instanceof IUpgradeTile upgradeTile) {
         if (tile instanceof Upgrade.IUpgradeInfoHandler upgradeInfoHandler) {
   ...
   public static List<Component> getMultScaledInfo(IUpgradeTile tile, Upgrade upgrade) {
      ...
      if (tile.supportsUpgrades() && upgrade.getMax() > 1) {
         double effect = Math.pow(MekanismConfig.general.maxUpgradeMultiplier.get(), (float)tile.getComponent().getUpgrades(upgrade) / upgrade.getMax());
         ret.add(MekanismLang.UPGRADES_EFFECT.translate(new Object[]{(float)Math.round(effect * 100.0) / 100.0F}));
      }
   ...
   public static List<Component> getExpScaledInfo(IUpgradeTile tile, Upgrade upgrade) {
      List<Component> ret = new ArrayList<>();
      if (tile.supportsUpgrades() && upgrade.getMax() > 1) {
         ret.add(MekanismLang.UPGRADES_EFFECT.translate(new Object[]{Math.pow(2.0, tile.getComponent().getUpgrades(upgrade))}));
      }
