...
public class FrequencyType<FREQ extends Frequency> {
   ...
   public static final FrequencyType<TeleporterFrequency> TELEPORTER = register(
      "Teleporter",
   ...
   public static final FrequencyType<InventoryFrequency> INVENTORY = register(
      "Inventory",
   ...
   public static final FrequencyType<SecurityFrequency> SECURITY = register(
      "Security", (key, uuid) -> new SecurityFrequency(uuid), SecurityFrequency::new, FrequencyManagerWrapper.Type.PUBLIC_ONLY, IdentitySerializer.UUID
   );
   public static final FrequencyType<QIOFrequency> QIO = register(
      "QIO", (key, uuid) -> new QIOFrequency((String)key, uuid), QIOFrequency::new, FrequencyManagerWrapper.Type.PUBLIC_PRIVATE, IdentitySerializer.NAME
   );
   ...
   private static <FREQ extends Frequency> FrequencyType<FREQ> register(
      String name,
      BiFunction<Object, UUID, FREQ> creationFunction,
      Supplier<FREQ> baseCreationFunction,
      FrequencyManagerWrapper.Type managerType,
   ...
   public FREQ create(Object key, UUID ownerUUID) {
      return this.creationFunction.apply(key, ownerUUID);
   }
   ...
   public FrequencyManager<FREQ> getFrequencyManager(@Nullable FREQ freq) {
      if (freq == null) {
         ...
         return freq.isPublic() ? this.getManagerWrapper().getPublicManager() : this.getManagerWrapper().getPrivateManager(freq.getOwner());
      }
   }
   ...
   public static <FREQ extends Frequency> FrequencyType<FREQ> load(CompoundTag tag) {
      return (FrequencyType<FREQ>)registryMap.get(tag.m_128461_("type"));
   }
