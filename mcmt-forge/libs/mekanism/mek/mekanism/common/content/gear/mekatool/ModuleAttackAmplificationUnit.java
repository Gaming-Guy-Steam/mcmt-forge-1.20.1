...
public class ModuleAttackAmplificationUnit implements ICustomModule<ModuleAttackAmplificationUnit> {
   private IModuleConfigItem<ModuleAttackAmplificationUnit.AttackDamage> attackDamage;

   @Override
   public void init(IModule<ModuleAttackAmplificationUnit> module, ModuleConfigItemCreator configItemCreator) {
      this.attackDamage = configItemCreator.createConfigItem(
         "attack_damage",
         MekanismLang.MODULE_BONUS_ATTACK_DAMAGE,
         new ModuleEnumData<>(ModuleAttackAmplificationUnit.AttackDamage.MED, module.getInstalledCount() + 2)
      );
   }
   ...
   public void addHUDStrings(IModule<ModuleAttackAmplificationUnit> module, Player player, Consumer<Component> hudStringAdder) {
      if (module.isEnabled()) {
         hudStringAdder.accept(
            MekanismLang.MODULE_DAMAGE.translateColored(EnumColor.DARK_GRAY, new Object[]{EnumColor.INDIGO, this.attackDamage.get().getDamage()})
         );
      }
   }
