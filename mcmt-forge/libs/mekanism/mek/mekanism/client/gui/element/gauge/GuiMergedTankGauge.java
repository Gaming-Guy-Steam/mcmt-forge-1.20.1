...
public class GuiMergedTankGauge<HANDLER extends IMekanismFluidHandler & IGasTracker & IInfusionTracker & IPigmentTracker & ISlurryTracker>
   ...
   public GuiMergedTankGauge(
      Supplier<MergedTank> mergedTankSupplier, Supplier<HANDLER> handlerSupplier, GaugeType type, IGuiWrapper gui, int x, int y, int width, int height
   ) {
      super(type, gui, x, y, width, height);
      this.mergedTankSupplier = mergedTankSupplier;
      this.handlerSupplier = handlerSupplier;
      this.fluidGauge = this.addPositionOnlyChild(
         new GuiFluidGauge(
            () -> this.mergedTankSupplier.get().getFluidTank(), () -> this.handlerSupplier.get().getFluidTanks(null), type, gui, x, y, width, height
         )
      );
   ...
   public GuiElement mouseClickedNested(double mouseX, double mouseY, int button) {
      ...
      if (currentGauge == null) {
         boolean clicked = this.fluidGauge.m_6375_(mouseX, mouseY, button)
            | this.gasGauge.m_6375_(mouseX, mouseY, button)
   ...
   private GuiTankGauge<?, ?> getCurrentGauge() {
      GuiTankGauge<?, ?> currentGauge = this.getCurrentGaugeNoFallback();
      return (GuiTankGauge<?, ?>)(currentGauge == null ? this.fluidGauge : currentGauge);
   }
   ...
   private GuiTankGauge<?, ?> getCurrentGaugeNoFallback() {
      ...
      return (GuiTankGauge<?, ?>)(switch (mergedTank.getCurrentType()) {
         case FLUID -> this.fluidGauge;
