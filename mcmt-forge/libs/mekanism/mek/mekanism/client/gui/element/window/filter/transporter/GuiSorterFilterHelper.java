...
public interface GuiSorterFilterHelper extends GuiFilterHelper<TileEntityLogisticalSorter>, IFancyFontRenderer {
   default void addSorterDefaults(
      IGuiWrapper gui,
      SorterFilter<?> filter,
      int slotOffset,
      UnaryOperator<GuiElement> childAdder,
      BooleanSupplier singleItem,
      ...
      childAdder.apply(
         new MekanismImageButton(
            ...
            relativeY + 18,
            11,
            MekanismUtils.getResource(MekanismUtils.ResourceType.GUI_BUTTON, "default.png"),
            () -> filter.allowDefault = !filter.allowDefault,
            (onHover, guiGraphics, mouseX, mouseY) -> gui.displayTooltips(
               guiGraphics, mouseX, mouseY, MekanismLang.FILTER_ALLOW_DEFAULT.translate(new Object[0])
            )
         )
      );
      ...
      childAdder.apply(
         new MekanismImageButton(
            ...
            11,
            14,
            MekanismUtils.getResource(MekanismUtils.ResourceType.GUI_BUTTON, "silk_touch.png"),
            () -> filter.sizeMode = !filter.sizeMode,
            (onHover, guiGraphics, mouseX, mouseY) -> {
               if (singleItem.getAsBoolean() && filter.sizeMode) {
                  gui.displayTooltips(guiGraphics, mouseX, mouseY, MekanismLang.SORTER_SIZE_MODE_CONFLICT.translate(new Object[0]));
               } else {
                  gui.displayTooltips(guiGraphics, mouseX, mouseY, MekanismLang.SORTER_SIZE_MODE.translate(new Object[0]));
               }
            }
         )
      );
   ...
   default void renderSorterForeground(GuiGraphics guiGraphics, SorterFilter<?> filter, boolean singleItem) {
      int relativeX = this.getRelativeX();
      int relativeY = this.getRelativeY();
      this.drawString(guiGraphics, BooleanStateDisplay.OnOff.of(filter.allowDefault).getTextComponent(), relativeX + 161, relativeY + 20, this.titleTextColor());
      this.drawString(guiGraphics, MekanismLang.MIN.translate(new Object[]{""}), relativeX + 148, relativeY + 32, this.titleTextColor());
      this.drawString(guiGraphics, MekanismLang.MAX.translate(new Object[]{""}), relativeX + 148, relativeY + 44, this.titleTextColor());
      if (singleItem && filter.sizeMode) {
         this.drawString(
            guiGraphics,
            MekanismLang.SORTER_FILTER_SIZE_MODE.translateColored(EnumColor.RED, new Object[]{BooleanStateDisplay.OnOff.of(true)}),
            relativeX + 161,
            relativeY + 58,
            this.titleTextColor()
         );
      } else {
         this.drawString(guiGraphics, BooleanStateDisplay.OnOff.of(filter.sizeMode).getTextComponent(), relativeX + 161, relativeY + 58, this.titleTextColor());
      }
   }
}
